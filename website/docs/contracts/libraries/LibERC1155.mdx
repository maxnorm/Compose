...
LibERC1155 is a utility library for managing the core functionality of ERC-1155 tokens in a Compose diamond architecture. It provides highly efficient and composable methods for minting, burning, transferring, and setting URIs, tailored to work seamlessly with the diamond storage pattern for scalable and modular contracts.


[View Source](https://github.com/maxnorm/Compose/blob/38545161423dd99e31b48b0eff41d260712fe731/src/token/ERC1155/LibERC1155.sol)
...
| `mint` | `` | Mints a single token type to an address. Increases the balance and emits a TransferSingle event. Performs receiver validation if recipient is a contract. |
...
| `mintBatch` | `` | Mints multiple token types to an address in a single transaction. Increases balances for each token type and emits a TransferBatch event. Performs receiver validation if recipient is a contract. |
...
| `burn` | `` | Burns a single token type from an address. Decreases the balance and emits a TransferSingle event. Reverts if the account has insufficient balance. |
...
| `burnBatch` | `` | Burns multiple token types from an address in a single transaction. Decreases balances for each token type and emits a TransferBatch event. Reverts if the account has insufficient balance for any token type. |
...
| `safeTransferFrom` | `` | Safely transfers a single token type from one address to another. Validates ownership, approval, and receiver address before updating balances. Performs ERC1155Receiver validation if recipient is a contract (safe transfer). Complies with EIP-1155 safe transfer requirements. |
...
| `setTokenURI` | `` | Sets the token-specific URI for a given token ID. Sets tokenURIs[_tokenId] to the provided string and emits a URI event with the full computed URI. The emitted URI is the concatenation of baseURI and the token-specific URI. |
...
| `setBaseURI` | `` | Sets the base URI prefix for token-specific URIs. The base URI is concatenated with token-specific URIs set via setTokenURI. Does not affect the default URI used when no token-specific URI is set. |
...
| `TransferSingle` | `` | Emitted when a single token type is transferred. |
...
| `TransferBatch` | `` | Emitted when multiple token types are transferred. |
...
| `URI` | `` | Emitted when the URI for token type `_id` changes to `_value`. |
...
- Always fetch the storage struct using `getStorage` every time before accessing or modifying ERC-1155 state.
- Validate ownership and approvals before invoking transfer or burn methods.
- Follow EIP-1155 guidelines for safe transfers using `safeTransferFrom` and `safeBatchTransferFrom` to ensure compatibility with the standard.
</Callout>
...
LibERC1155 operates directly on a predefined diamond storage slot, ensuring all updates are visible automatically to other facets in the diamond that access the same storage. This design enables safe and efficient state sharing across the diamond while maintaining modularity.
</Callout>
