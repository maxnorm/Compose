...
LibERC1155 is a library for implementing EIP-1155 multi-token standard in the context of diamond contracts. It provides efficient and reusable functions to manage token minting, burning, transferring, and metadata, while ensuring compatibility with the diamond storage pattern and safe transfer requirements of the standard.


[View Source](https://github.com/maxnorm/Compose/blob/e3e377cf3d77e94dc30e10812da2bdd1907ca159/src/token/ERC1155/LibERC1155.sol)
...
contract MyDiamond {
    ...
    function getTokenURI(uint256 _id) external view returns (string memory) {
        return LibERC1155.getStorage().tokenURIs[_id];
    }
}
```
...
- Always validate ownership and approval before calling transfer or burn functions to avoid unexpected reverts.
- Use `mintBatch` and `burnBatch` for bulk token operations to save gas costs.
- Avoid modifying diamond storage directly; rely on provided library functions to maintain consistency and correctness.
</Callout>
...
LibERC1155 leverages the diamond storage pattern to store and manage token state, ensuring compatibility and visibility across all facets within a diamond contract. Changes made through this library are immediately reflected in any ERC-1155 facet functions using the same storage.
</Callout>
