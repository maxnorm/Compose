...
gitSource: "https://github.com/maxnorm/Compose/blob/35c4fb10b73eff0850482e188d0f1eca595cff2c/src/access/AccessControlPausable/AccessControlPausableMod.sol"
---
...
- Supports pausing and unpausing of specific roles.
- Error handling via custom errors for unauthorized accounts or paused roles.
- Provides robust role-based access control integration for Compose diamonds.
</Callout>
...
AccessControlPausableMod provides role-based access control with support for pausing and unpausing specific roles. This module enhances security and flexibility in Compose diamonds by allowing facets to enforce role-specific restrictions dynamically, ensuring robust access management.
...
contract ExampleFacet {
    AccessControlPausableMod private accessControl;

    constructor(address _accessControlAddress) {
        accessControl = AccessControlPausableMod(_accessControlAddress);
    }

    function restrictedAction(bytes32 _role) external {
        accessControl.requireRoleNotPaused(msg.sender, _role);
        // Perform action
    }

    function pauseSpecificRole(bytes32 _role) external {
        accessControl.pauseRole(_role);
    }

    function unpauseSpecificRole(bytes32 _role) external {
...
- Ensure role pausing is used judiciously to avoid unintentionally blocking access.
- Validate roles and their states before executing critical functions.
- Use &#x60;requireRoleNotPaused&#x60; to prevent unauthorized or paused role execution dynamically.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
AccessControlPausableMod utilizes diamond storage to manage its state, ensuring compatibility with the diamond proxy pattern. Changes to roles (pause/unpause) are immediately reflected across facets that rely on this module. Ensure facets interacting with this module implement consistent role definitions to maintain predictable behavior.
</Callout>
...
<LastUpdated date="2025-12-17T00:37:09.555Z" />
