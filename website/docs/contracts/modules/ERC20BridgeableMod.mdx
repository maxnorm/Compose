...
- Enforces strict access control, allowing only addresses with the &#x60;trusted-bridge&#x60; role to perform cross-chain actions.
- Includes internal checks to validate bridge callers, preventing unauthorized operations.
</Callout>
...
The ERC20BridgeableMod module provides cross-chain token transfer functionality. It enforces access control to ensure only trusted bridges can mint or burn tokens, enhancing security for cross-chain operations. This module is crucial for maintaining the integrity of token balances across different networks.
...
contract MyFacet {
    IERC20BridgeableMod internal constant ERC20_BRIDGEABLE_MODULE = IERC20BridgeableMod(address(0)); // Placeholder, will be the actual module address

    function _mintCrosschain(address _to, uint256 _amount) internal {
        // Assume AccessControl is handled elsewhere and caller has 'trusted-bridge' role
        ERC20_BRIDGEABLE_MODULE.crosschainMint(_to, _amount);
    }

    function _burnCrosschain(address _from, uint256 _amount) internal {
        // Assume AccessControl is handled elsewhere and caller has 'trusted-bridge' role
        ERC20_BRIDGEABLE_MODULE.crosschainBurn(_from, _amount);
    }
}`}
</ExpandableCode>
...
- Ensure the caller has the &#x60;trusted-bridge&#x60; role before invoking &#x60;crosschainMint&#x60; or &#x60;crosschainBurn&#x60;.
- Always validate receiver addresses to prevent sending tokens to invalid destinations (e.g., zero address). The &#x60;checkTokenBridge&#x60; internal function can be leveraged for this purpose.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module interacts with the diamond&#x27;s storage to manage access control and ERC20 token data. The &#x60;getAccessControlStorage&#x60; and &#x60;getERC20Storage&#x60; functions provide direct access to these critical storage areas. Facets using this module should be aware that any changes to the access control list or ERC20 token configurations directly impact the behavior of cross-chain operations. The module relies on the presence of a correctly initialized AccessControl contract within the diamond.
</Callout>
...
<LastUpdated date="2025-12-19T19:09:45.221Z" />
