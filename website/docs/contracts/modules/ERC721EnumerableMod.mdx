...
gitSource: "https://github.com/maxnorm/Compose/blob/117a9e586e335bd5662e5aba533c315582d9ef9b/src/token/ERC721/ERC721Enumerable/ERC721EnumerableMod.sol"
---
...
- Safely integrates ERC-721 enumerable state into Compose diamonds using diamond storage.
- Implements complete mint, burn, and transfer functionality while updating enumeration lists.
- Reverts automatically on invalid operations (e.g., nonexistent tokens, unauthorized transfers).
</Callout>
...
ERC721EnumerableMod provides enumeration utilities for ERC-721 tokens in a Compose diamond proxy. It enables facets to mint, burn, and transfer tokens while maintaining correct ownership and enumerable records, ensuring safe and coherent state management.
...
contract MyERC721Facet {
    ...
    function fetchStorage() external view returns (ERC721EnumerableMod.Storage storage) {
...
- Validate that mint and burn operations are restricted to authorized roles or entities.
- Ensure correct diamond storage struct usage by interacting with getStorage to avoid state inconsistencies.
- Check token existence within custom logic to handle errors gracefully when using burn or transferFrom.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
ERC721EnumerableMod uses a specific slot in diamond storage to manage token enumeration data. Access this storage via the getStorage method when implementing facets. Ensure that any modifications to storage comply with existing data layout to maintain cross-facet compatibility.
</Callout>
...
<LastUpdated date="2025-12-15T20:58:34.842Z" />
