...
The AccessControlTemporalMod module enhances diamond proxy functionality by introducing time-bound access control. It enables roles to be assigned and automatically expire after a specified duration, improving security and flexibility when managing permissions in smart contracts.


[View Source](https://github.com/maxnorm/Compose/blob/eabc77b81e461d751a5a8e003c3a9c2e818b9d3c/src/access/AccessControlTemporal/AccessControlTemporalMod.sol)



<Callout type="info" title="Key Features">
- Time-bound role assignments for precise permission control.\n- Simple APIs for granting, revoking, and validating role assignments.\n- Built-in error checks for expired or unauthorized roles.
</Callout>
...
```solidity
pragma solidity ^0.8.0;\n\nimport { AccessControlTemporalMod } from &quot;path/to/AccessControlTemporalMod.sol&quot;;\n\ncontract ExampleUsage {\n    AccessControlTemporalMod private accessControl;\n\n    constructor(address _diamond) {\n        accessControl = AccessControlTemporalMod(_diamond);\n    }\n\n    function grantRoleWithTimeout(bytes32 _role, address _account, uint256 _expiry) external {\n        accessControl.grantRoleWithExpiry(_role, _account, _expiry);\n    }\n\n    function checkRoleValidity(bytes32 _role, address _account) external view returns (bool) {\n        return !accessControl.isRoleExpired(_role, _account);\n    }\n\n    function revokeRole(bytes32 _role, address _account) external {\n        accessControl.revokeTemporalRole(_role, _account);\n    }\n}
```
...
- Always validate that the expiry timestamp provided in grantRoleWithExpiry is reasonable and aligns with application security requirements.\n- Regularly review and audit the roles granted with expiry to ensure compliance with permissions and security standards.\n- Revoke unused or unnecessary roles promptly using revokeTemporalRole to minimize the attack surface.
</Callout>
...
This module relies on the diamond storage pattern for managing access control data, ensuring seamless access and compatibility with other facets. Changes made through this module, such as granting or revoking roles, directly update the underlying shared storage and are immediately visible to other facets leveraging the same storage.
</Callout>
