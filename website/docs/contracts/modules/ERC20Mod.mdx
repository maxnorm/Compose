...
gitSource: "https://github.com/maxnorm/Compose/blob/35c4fb10b73eff0850482e188d0f1eca595cff2c/src/token/ERC20/ERC20/ERC20Mod.sol"
---
...
- Implements core ERC-20 standards including mint, burn, transfer, and approval.
- Uses fixed diamond storage position for consistent state management.
- Supports balance and allowance updates directly within the diamond storage.
</Callout>
...
ERC20Mod provides core ERC-20 functionality for Compose diamonds, including token transfers, minting, burning, and approvals. It ensures efficient storage reuse by binding the ERC-20 state to a fixed diamond storage position, enabling seamless integration across facets.
...
contract MyERC20Facet {
    ...
    function burnTokens(uint256 _amount) external {
        ERC20Mod.burn(msg.sender, _amount);
    }
}`}
</ExpandableCode>
...
- Always ensure privileges (e.g., minting) are properly gated to authorized addresses.
- Use the &#x60;getStorage&#x60; function to interact with shared ERC-20 state to prevent storage collision.
- Validate input parameters (e.g., non-zero addresses and correct allowance amounts) to avoid unexpected behavior.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
ERC20Mod interacts with a shared ERC-20 storage struct bound to a fixed storage slot in the diamond. Facets must use the &#x60;getStorage&#x60; function to access and manage the token state, ensuring consistency. Facets must maintain functional separation, with minting and burning operations restricted to authorized callers to uphold security guarantees. Any updates to the ERC-20 storage are immediately visible to all facets using the same storage structure.
</Callout>
...
<LastUpdated date="2025-12-17T00:38:18.412Z" />
