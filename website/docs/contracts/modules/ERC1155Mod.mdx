...
gitSource: "https://github.com/maxnorm/Compose/blob/56196d950dad5f803c271d96961275928a79b519/src/token/ERC1155/ERC1155Mod.sol"
---
...
- Implements EIP-1155 safe transfer and receiver validation.
- Supports batch operations for minting, burning, and transfers, reducing gas overhead when managing multiple token types.
- Provides flexible URI management with base and token-specific URIs for metadata consistency.
</Callout>
...
The ERC1155Mod module implements the core functionality for handling ERC-1155 token transfers, minting, burning, and URI management within a diamond proxy architecture. It enables reusable and composable token logic by adhering to EIP-1155 standards while leveraging diamond storage for state efficiency and safety.
...
contract ExampleFacet {
    ERC1155Mod private immutable erc1155;

    constructor(address _erc1155Mod) {
        erc1155 = ERC1155Mod(_erc1155Mod);
    }
...
- Ensure sufficient access control for minting and burning functions to prevent misuse.
- Validate recipient contract compliance with the ERC1155Receiver interface before completing safe transfers.
- Follow proper diamond upgrade patterns when modifying the storage struct via getStorage to avoid storage slot conflicts.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
The ERC1155Mod module manipulates diamond storage using the getStorage function, ensuring compatibility with the diamond storage pattern. Changes to token balances or URIs are immediately visible to facets using the same storage layout. Ensure proper ordering of calls involving URI or token state modifications, as intermediate state changes can affect subsequent behavior. Maintain consistent use of storage slot references to uphold module integrity during upgrades.
</Callout>
...
<LastUpdated date="2025-12-16T00:45:13.663Z" />
