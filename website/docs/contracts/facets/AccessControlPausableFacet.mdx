...
gitSource: "https://github.com/maxnorm/Compose/blob/56196d950dad5f803c271d96961275928a79b519/src/access/AccessControlPausable/AccessControlPausableFacet.sol"
---
...
- Allows administrators to pause or unpause specific roles for better control during critical scenarios.
- Guarantees that actions tied to paused roles are blocked until explicitly re-enabled.
</Callout>

## Overview

The AccessControlPausableFacet provides functionality to manage access control with role-based pause mechanisms. It allows administrators to temporarily disable specific roles, preventing their associated actions, and re-enable them later. This helps in fine-grained permission management and system safety during critical operations.
...
interface IAccessControlPausableFacet {
    function pauseRole(bytes32 _role) external;
    function unpauseRole(bytes32 _role) external;
    function isRolePaused(bytes32 _role) external view returns (bool);
}
...
contract AccessControlExample {
    address diamondAddress = 0x1234567890abcdef1234567890abcdef12345678;

    function pauseUserRole(bytes32 role) external {
        IAccessControlPausableFacet(diamondAddress).pauseRole(role);
    }

    function unpauseUserRole(bytes32 role) external {
        IAccessControlPausableFacet(diamondAddress).unpauseRole(role);
    }

    function checkUserRolePaused(bytes32 role) external view returns (bool) {
        return IAccessControlPausableFacet(diamondAddress).isRolePaused(role);
    }
}`}
</ExpandableCode>
...
- Ensure the facet is initialized properly if it modifies storage; call any required init function during the diamond upgrade.
- Maintain strict role definitions and assign admin roles to trusted accounts only.
- Logically group pausable roles to avoid over-complicating pause/unpause management.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Verify that only the admin of a role can pause or unpause it, ensuring access control integrity. Ensure proper input validation for role identifiers to prevent misuse or unintended behavior. Avoid reentrancy issues by following the checks-effects-interactions pattern during state updates.
</Callout>
...
<LastUpdated date="2025-12-16T00:43:34.031Z" />
