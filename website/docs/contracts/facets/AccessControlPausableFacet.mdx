...
gitSource: "https://github.com/maxnorm/Compose/blob/326d9e4b8d8f7ef437a77f59f91bb1619c2d1a6c/src/access/AccessControlPausable/AccessControlPausableFacet.sol"
---
...
- Provides both access control and role pausing mechanisms for better granularity of permission management.
- Only admins of roles are authorized to pause or unpause a specific role.
</Callout>

## Overview

AccessControlPausableFacet enables fine-grained access control with role pausing functionality within a diamond. It allows temporary suspension of role permissions and ensures restricted function access using role and pause state evaluations.
...
contract DeployDiamond {
    function addAccessControlPausableFacet(address diamondAddress, address facetAddress) external {
    ...
    function usePausedRoleFunction(address accessControlPausableFacet, bytes32 role) external {
        require(!AccessControlPausableFacet(accessControlPausableFacet).isRolePaused(role), "Role is paused");
        // Perform actions restricted to the role here
    }
}

library AccessControlPausableFacet {
    struct RoleData {
        bool paused;
        address admin;
    }
    ...
    function getAccessControlStorage() external pure returns (AccessControlStorage storage ds) {
        // Example storage slot calculation omitted
    }
}`}
</ExpandableCode>
...
- Always initialize role admin at the time of setup to prevent misuse.
- Utilize role pausing for critical updates or emergency scenarios to avoid misuse of permissions.
- Verify role and admin assignments during and after a diamond upgrade to maintain security guarantees.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Ensure role admin permissions are properly assigned to trusted accounts to prevent unauthorized pausing or unpausing. Validate all inputs to avoid potential invalid state changes. Be cautious of indirect access through other modules that could bypass the role pausing checks.
</Callout>
...
<LastUpdated date="2025-12-15T21:45:43.755Z" />
