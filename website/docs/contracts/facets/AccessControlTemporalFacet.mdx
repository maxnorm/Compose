...
gitSource: "https://github.com/maxnorm/Compose/blob/326d9e4b8d8f7ef437a77f59f91bb1619c2d1a6c/src/access/AccessControlTemporal/AccessControlTemporalFacet.sol"
---
...
- Roles can be assigned with customizable expiry timestamps.
- Explicit checks for expired roles using isRoleExpired.
- Emits audit-friendly events for role grants and revocations.
</Callout>

## Overview

AccessControlTemporalFacet provides time-limited access control capabilities within a diamond proxy architecture. It enables the assignment of roles with expiry timestamps, verification of role validity, and the revocation of expired or invalid roles.
...
interface IDiamond {
    function execute(bytes4 selector, bytes calldata data) external payable;
}
...
contract ExampleUsage {
    ...
    function grantTemporalRole(address account, bytes32 role, uint256 expiry) external {
        bytes memory grantRoleData = abi.encodeWithSelector(
            bytes4(keccak256("grantRoleWithExpiry(address,bytes32,uint256)")),
            account,
            role,
            expiry
        );
        diamond.execute(bytes4(keccak256("grantRoleWithExpiry(address,bytes32,uint256)")), grantRoleData);
    }

    function checkValidRole(address account, bytes32 role) external {
        bytes memory requireValidRoleData = abi.encodeWithSelector(
            bytes4(keccak256("requireValidRole(address,bytes32)")),
            account,
            role
        );
        diamond.execute(bytes4(keccak256("requireValidRole(address,bytes32)")), requireValidRoleData);
    }

    function revokeTemporalRole(address account, bytes32 role) external {
        bytes memory revokeRoleData = abi.encodeWithSelector(
            bytes4(keccak256("revokeTemporalRole(address,bytes32)")),
            account,
            role
        );
        diamond.execute(bytes4(keccak256("revokeTemporalRole(address,bytes32)")), revokeRoleData);
    }
}`}
</ExpandableCode>
...
- Ensure only authorized admin accounts invoke grantRoleWithExpiry and revokeTemporalRole.
- Validate expiry timestamps are in the future when granting a role.
- Use requireValidRole where role validity can impact critical diamond behavior.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Restrict grantRoleWithExpiry and revokeTemporalRole to authorized role admins. Validate expiry timestamps to prevent unintended role durations. Use requireValidRole judiciously to avoid unauthorized actions tied to expired roles.
</Callout>
...
<LastUpdated date="2025-12-15T21:45:50.698Z" />
