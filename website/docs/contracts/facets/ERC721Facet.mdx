...
title: "ERC721Facet"
description: "**Title:**"
gitSource: "https://github.com/maxnorm/Compose/blob/117a9e586e335bd5662e5aba533c315582d9ef9b/src/token/ERC721/ERC721/ERC721Facet.sol"
---
...
- Implements full ERC-721 compliance, including token metadata, approvals, and safe transfers.
- Efficient storage access via inline assembly and diamond storage.
- Internal utilities like internalTransferFrom facilitate controlled logic without exposing extraneous entry points.
</Callout>

## Overview

The ERC721Facet implements core ERC-721 token functionality within a Compose diamond. It provides token metadata retrieval, ownership management, approval handling, and token transfers while abstracting storage operations via the diamond storage pattern. This facet surfaces all standard ERC-721 callable selectors to enable seamless integration.
...
contract ERC721Diamond {
    function deployERC721Facet() external {
        // Assume \`diamondCutAddress\` implements \`IDiamondCut\`
        IDiamondCut diamondCut = IDiamondCut(diamondCutAddress);

        // Configure ERC721Facet selectors
        bytes4[] memory selectors = new bytes4[](13);
        selectors[0] = ERC721Facet.getStorage.selector;
        ...
        selectors[6] = ERC721Facet.getApproved.selector;
        selectors[7] = ERC721Facet.isApprovedForAll.selector;
        selectors[8] = ERC721Facet.approve.selector;
        selectors[9] = ERC721Facet.setApprovalForAll.selector;
        selectors[10] = ERC721Facet.transferFrom.selector;
        selectors[11] = ERC721Facet.safeTransferFrom.selector;
        selectors[12] = ERC721Facet.safeTransferFrom.selector;

        // Add ERC721Facet to the diamond
        diamondCut.diamondCut([{facetAddress: address(new ERC721Facet()), action: IDiamondCut.FacetCutAction.Add, functionSelectors: selectors}], address(0), "");
    }

    function useERC721Functions() external {
        ...
        address owner = erc721Facet.ownerOf(123);
        uint256 balance = erc721Facet.balanceOf(msg.sender);
        erc721Facet.transferFrom(msg.sender, recipient, 123);
    }
}`}
</ExpandableCode>
...
- Initialize ERC721Facet via diamondCut before calling its functions.
- Ensure storage layout correctness and preservation for upgrades using the diamond storage pattern.
- Perform access control checks for administrative tasks like diamondCut deployments.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Verify ownership and operator approvals carefully to prevent unauthorized transfers. Ensure proper input validation for token IDs and addresses, especially in transfer and approval functions. Safeguard against reentrancy in &#x60;safeTransferFrom&#x60; by invoking external calls last.
</Callout>
...
<LastUpdated date="2025-12-15T20:59:17.865Z" />
