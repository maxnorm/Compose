...
- Supports granting and revoking roles to/from multiple addresses efficiently.
- Defines a &#x60;DEFAULT_ADMIN_ROLE&#x60; for bootstrapping and managing other roles.
- Includes explicit error types for unauthorized access.
</Callout>

## Overview

The AccessControlFacet manages role-based access control (RBAC) within a Compose diamond. It allows defining roles, assigning them to addresses, and enforcing role-based permissions on function calls, acting as a central authorization layer for the diamond.
...
contract DeployDiamond is DiamondInit {
    function deploy() external {
        // ... other facet deployments ...

        // Deploy AccessControlFacet
        address accessControlFacetAddress = address(new AccessControlFacet());
        addFacet(accessControlFacetAddress);

        // Initialize AccessControlFacet (example: setting default admin)
        bytes memory initData = abi.encodeWithSignature(\"initialize(address)\", msg.sender);
        addFacetAndInit(accessControlFacetAddress, initData);

        // Grant roles
        AccessControlFacet acf = AccessControlFacet(diamondAddress());
        bytes32 adminRole = acf.getRoleAdmin(acf.DEFAULT_ADMIN_ROLE());
        acf.grantRole(adminRole, msg.sender); // Grant DEFAULT_ADMIN_ROLE to deployer
        acf.grantRole(acf.DEFAULT_ADMIN_ROLE(), address(this)); // Grant DEFAULT_ADMIN_ROLE to the diamond itself for future upgrades

        // Grant a custom role
        bytes32 customRole = keccak256(\"CUSTOM_ROLE\");
        acf.grantRole(customRole, 0x1234567890123456789012345678901234567890);
    }
...
- Initialize the &#x60;AccessControlFacet&#x60; with the &#x60;DEFAULT_ADMIN_ROLE&#x60; assigned to the deployer or an appropriate multisig.
- Grant the &#x60;DEFAULT_ADMIN_ROLE&#x60; to the diamond proxy itself or a designated upgrade manager for seamless upgrades.
- Use &#x60;requireRole&#x60; or &#x60;hasRole&#x60; checks within other facets to enforce permissions, compositing access control logic.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Ensure that the caller granting or revoking roles possesses the administrative role for the target role. The &#x60;renounceRole&#x60; function allows an account to give up a role, which should be used with caution. Input validation is critical; always ensure addresses and role identifiers are correctly formatted. Reentrancy is mitigated by the diamond proxy pattern and the function design, but external calls within role management functions should be carefully reviewed.
</Callout>
...
<LastUpdated date="2025-12-19T19:09:21.709Z" />
