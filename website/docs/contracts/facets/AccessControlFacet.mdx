...
gitSource: "https://github.com/maxnorm/Compose/blob/c96b0cfc6f3d9e9ed44be16f0013d67627238281/src/access/AccessControl/AccessControlFacet.sol"
---
...
- Supports role creation, assignment, and revocation with fine-grained control.
- Provides admin role hierarchy to delegate role management securely.
</Callout>

## Overview

The AccessControlFacet provides fine-grained role-based access control functionalities within a Compose diamond. It facilitates managing roles, assigning or revoking them, and handling role hierarchy via admin roles, enabling modular and secure permission control.
...
contract ExampleDiamond {
    ...
    function exampleRoleUsage() external {
        // Grant a role
        bytes memory callDataGrant = abi.encodeWithSignature(\"grantRole(bytes32,address)\",
            keccak256(\"MY_ROLE\"),
            msg.sender
        );
        (bool successGrant, ) = diamondAddress.call(callDataGrant);
        require(successGrant, \"grantRole failed\");

        // Check if an account has a role
        bytes memory callDataHasRole = abi.encodeWithSignature(\"hasRole(bytes32,address)\",
            keccak256(\"MY_ROLE\"),
            msg.sender
        );
        (bool successHasRole, bytes memory resultHasRole) = diamondAddress.call(callDataHasRole);
        require(successHasRole, \"hasRole failed\");
        bool hasRole = abi.decode(resultHasRole, (bool));
        require(hasRole, \"Account does not have MY_ROLE\");

        // Revoke a role
        bytes memory callDataRevoke = abi.encodeWithSignature(\"revokeRole(bytes32,address)\",
            keccak256(\"MY_ROLE\"),
            msg.sender
        );
        (bool successRevoke, ) = diamondAddress.call(callDataRevoke);
        require(successRevoke, \"revokeRole failed\");
    }
}`}
</ExpandableCode>
...
- Ensure each role&#x27;s admin role is correctly set using &#x60;setRoleAdmin&#x60; during initialization.
- Always verify whether the caller holds the required role before invoking sensitive transactions.
- Use batch operations (&#x60;grantRoleBatch&#x60; or &#x60;revokeRoleBatch&#x60;) for efficiency when managing multiple accounts.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Restrict access to admin functions (&#x60;setRoleAdmin&#x60;, &#x60;grantRole&#x60;, &#x60;revokeRole&#x60;, etc.) only to authorized roles. Validate input parameters for role consistency and completeness. Prevent reentrancy vulnerabilities if roles manage external state or interact with other contracts. Avoid privilege escalation by ensuring admin roles are correctly configured after deployment.
</Callout>
...
<LastUpdated date="2025-12-15T15:39:49.690Z" />
