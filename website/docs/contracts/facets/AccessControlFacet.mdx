...
gitSource: "https://github.com/maxnorm/Compose/blob/35c4fb10b73eff0850482e188d0f1eca595cff2c/src/access/AccessControl/AccessControlFacet.sol"
---
...
- Fine-grained role-based access control with support for custom roles.
- Batch operations for granting or revoking roles to/from multiple accounts.
- Role hierarchy through admin roles allows layered access control management.
</Callout>

## Overview

The AccessControlFacet provides role-based access control functionality within the Compose diamond architecture. It enables fine-grained permissions management through role assignment, revocation, and administrative oversight. This facet is essential for enforcing secure and modular access policies in a diamond proxy context.
...
contract AccessControlExample {
    ...
    function setupRoles(address admin, address user) external {
        ...
        AccessControlFacet(address(diamond)).grantRole(keccak256("ADMIN_ROLE"), admin);

        // Grant user role to the user address
        AccessControlFacet(address(diamond)).grantRole(keccak256("USER_ROLE"), user);
    }

    function checkRole(address account, bytes32 role) external view returns (bool) {
        return AccessControlFacet(address(diamond)).hasRole(role, account);
    }

    function revokeUserRole(address user) external {
        // Revoke user role from the specified user
        AccessControlFacet(address(diamond)).revokeRole(keccak256("USER_ROLE"), user);
    }
}`}
</ExpandableCode>
...
- Always initialize the facet properly to set up initial roles and their corresponding admin roles.
- Use getStorage to access and verify the state of the AccessControl data before performing sensitive operations.
- Validate role existence and hierarchy before assigning or revoking roles to ensure correct access levels.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Ensure proper access control checks for functions like setRoleAdmin, grantRole, and revokeRole to prevent unauthorized changes. Validate inputs for role identifiers and account addresses. Be cautious of reentrancy risks when roles are granted or revoked in conjunction with other external calls.
</Callout>
...
<LastUpdated date="2025-12-17T00:37:52.571Z" />
