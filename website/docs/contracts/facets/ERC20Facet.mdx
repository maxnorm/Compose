...
gitSource: "https://github.com/maxnorm/Compose/blob/326d9e4b8d8f7ef437a77f59f91bb1619c2d1a6c/src/token/ERC20/ERC20/ERC20Facet.sol"
---
...
- Implements all standard ERC-20 functions for modular token management.
- Provides access to metadata, balances, allowances, and token transfers in a single facet.
- Designed for compatibility with the diamond proxy and storage patterns.
</Callout>

## Overview

ERC20Facet implements core ERC-20 token standard functionality within the Compose diamond architecture. It provides methods for querying balances, transferring tokens, managing allowances, and accessing ERC-20-specific metadata, such as name, symbol, and decimals. By encapsulating these operations in a facet, it enables decentralized token management with modular extensibility and upgradeability.
...
// Importing the diamond interface
import {IDiamondCut} from "./interfaces/IDiamondCut.sol";

contract DeployAndUseERC20Facet {
    address public diamondAddress;

    function deployDiamond(address diamondCutFacet, address erc20Facet) external {
        // Create the diamond
        IDiamondCut.FacetCut[] memory facetCuts = new IDiamondCut.FacetCut[](1);
        facetCuts[0] = IDiamondCut.FacetCut({
            facetAddress: erc20Facet,
            action: IDiamondCut.FacetCutAction.Add,
            functionSelectors: abi.encodePacked(keccak256("getStorage()"), keccak256("name()"), keccak256("symbol()"), keccak256("decimals()"), keccak256("totalSupply()"), keccak256("balanceOf(address)"), keccak256("allowance(address,address)"), keccak256("approve(address,uint256)"), keccak256("transfer(address,uint256)"), keccak256("transferFrom(address,address,uint256)"))
        });
    ...
    function exampleUsage() external {
        // Example: Interacting with the ERC20Facet in diamond
        IERC20Facet erc20 = IERC20Facet(diamondAddress);
        string memory tokenName = erc20.name();
        string memory tokenSymbol = erc20.symbol();
        erc20.approve(0xSpenderAddress, 1000);
        erc20.transfer(0xRecipientAddress, 500);
        uint256 balance = erc20.balanceOf(0xMyAddress);
        require(balance >= 500, "Not enough balance");
    }
}
...
- Ensure the correct diamond storage slot is used for ERC20Facet to avoid storage collisions.
- Use the diamond&#x27;s facet initialization flow to set up required initial state, like &#x60;name&#x60;, &#x60;symbol&#x60;, or &#x60;totalSupply&#x60;, before allowing interactions.
- Implement proper upgradability testing to ensure new facets do not override or unintentionally interfere with ERC20Facet&#x27;s state or functionality.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Ensure allowances and approvals are carefully implemented to prevent unexpected token usage or double spends.
Validate all input parameters such as recipient and spender addresses to prevent interactions with unintended accounts (e.g., zero address).
The &#x60;transfer&#x60; and &#x60;transferFrom&#x60; functions must handle reentrancy risks by ensuring state changes occur before external calls. Incorporate relevant checks to maintain consistency in the diamond storage.
</Callout>
...
<LastUpdated date="2025-12-15T21:46:57.686Z" />
