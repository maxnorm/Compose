...
[View Source](https://github.com/maxnorm/Compose/blob/21dc7260ede2a70f647074cc724c78ca08dfafb9/src/interfaces/IERC1155.sol)
...
| `setApprovalForAll` | `` | Grants or revokes permission to `operator` to transfer the caller's tokens. |
...
| `safeTransferFrom` | `` | Transfers `value` amount of token type `id` from `from` to `to`. |
...
| `uri` | `` | Returns the URI for token type `id`. |
