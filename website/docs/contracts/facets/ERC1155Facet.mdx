...
title: "ERC1155Facet"
description: "**Title:**"
gitSource: "https://github.com/maxnorm/Compose/blob/117a9e586e335bd5662e5aba533c315582d9ef9b/src/token/ERC1155/ERC1155Facet.sol"
---
...
- Full support for the ERC-1155 multi-token standard.
- Secure token transfers with event emissions ensure state integrity and traceability.
- Supports efficient batch operations for balance queries and transfers.
</Callout>

## Overview

The ERC1155Facet provides functionality for managing and interacting with ERC-1155 multi-token standards within a diamond proxy. It includes storage access, token balance queries, approval management, and secure token transfers, enabling efficient multi-token handling in a Compose diamond.
...
contract DeployDiamond {
    function deploy() external {
        ...
        facetCuts[0] = IDiamondCut.FacetCut({
            ...
            functionSelectors: getERC1155Selectors()
        });

        // Deploy and initialize diamond
        IDiamondCut(diamondAddress).diamondCut(facetCuts, address(0), hex'');

        // Example of invoking ERC1155Facet functions
        ERC1155Facet(diamondAddress).setApprovalForAll(address(0x1234), true);
        bool isApproved = ERC1155Facet(diamondAddress).isApprovedForAll(msg.sender, address(0x1234));
    }

    function getERC1155Selectors() internal pure returns (bytes4[] memory selectors) {
...
- Initialize default URI explicitly if token-specific URIs depend on baseURI concatenation.
- Use batched functions for efficiency when interacting with multiple accounts or tokens.
- Validate upgrade paths by ensuring compatibility of new selectors with existing facet integration.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Use setApprovalForAll cautiously to prevent unauthorized operators from transferring tokens.
Ensure reentrancy guards or proper design to mitigate reentrancy attacks in token transfer functions.
Validate inputs such as addresses, token IDs, and transfer amounts to prevent invalid or unintended operations.
</Callout>
...
<LastUpdated date="2025-12-15T20:58:10.747Z" />
