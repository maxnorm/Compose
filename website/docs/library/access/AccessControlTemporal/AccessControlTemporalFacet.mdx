...
- Utilizes Compose diamond storage pattern.
- Compatible with ERC-2535 diamond standard.
</Callout>

## Overview

This facet implements time-bound role assignments within a diamond. It provides functions to grant roles with specific expiry timestamps and to revoke them. Developers integrate this facet to manage temporary access privileges, ensuring roles automatically expire, reducing manual cleanup. Calls are routed through the diamond proxy to access the temporal access control storage.
...
- Initialize roles and grant initial temporal roles during diamond setup.
- Enforce role validity using `requireValidRole` before critical operations.
- Integrate with `OwnerTwoStepsFacet` or similar for secure role management.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
All state-changing functions (`grantRoleWithExpiry`, `revokeTemporalRole`) are protected by access control checks, reverting with `AccessControlUnauthorizedAccount` if the caller is not the role administrator. The `requireValidRole` function checks for both role existence and expiry, reverting with `AccessControlUnauthorizedAccount` or `AccessControlRoleExpired` respectively. Input validation for `_expiresAt` should be handled by the caller or a higher-level facet.
</Callout>
...
<LastUpdated date="2025-12-30T17:19:02.817Z" />
