...
gitSource: "https://github.com/maxnorm/Compose/blob/7e2dd824639f424be644c4ebbb3ca2508167329f/src/access/AccessControlTemporal/AccessControlTemporalFacet.sol"
---
...
- Automatically revokes expired roles upon validation checks.
- Provides functions to check if a role has expired or is currently valid.
- Requires administrative privileges for granting and revoking temporal roles.
</Callout>

## Overview

The AccessControlTemporalFacet extends the diamond's access control capabilities by introducing time-bound role assignments. It allows administrators to grant roles that automatically expire and provides mechanisms to check role validity, ensuring that access is dynamically managed based on time constraints.
...
import {AccessControlTemporalFacet} from "@compose/diamond-contracts/contracts/facets/AccessControl/AccessControlTemporalFacet.sol";

contract Deployer {
    address immutable DIAMOND_ADDRESS;
    address immutable ADMIN_ADDRESS;

    constructor(address diamondAddress, address adminAddress) {
        DIAMOND_ADDRESS = diamondAddress;
        ADMIN_ADDRESS = adminAddress;
    }

    function grantTemporalRole(bytes32 role, address account, uint64 expiry) external {
        AccessControlTemporalFacet(DIAMOND_ADDRESS).grantRoleWithExpiry(role, account, expiry);
    }

    function revokeTemporalRole(bytes32 role, address account) external {
        AccessControlTemporalFacet(DIAMOND_ADDRESS).revokeTemporalRole(role, account);
    }

    function isRoleValid(bytes32 role, address account) external view returns (bool) {
        return !AccessControlTemporalFacet(DIAMOND_ADDRESS).isRoleExpired(role, account);
    }
}`}
</ExpandableCode>
...
- Ensure the caller of `grantRoleWithExpiry` and `revokeTemporalRole` is the designated admin for the role to prevent unauthorized modifications.
- Regularly check role expiry using `isRoleExpired` or `requireValidRole` to maintain accurate access control.
- When upgrading facets, ensure that storage layout compatibility is maintained for `AccessControl` and `AccessControlTemporal` storage.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Access to functions like `grantRoleWithExpiry` and `revokeTemporalRole` is restricted to the role's administrator. The `requireValidRole` function enforces time-based access, reverting with `AccessControlRoleExpired` if a role has passed its expiry. Ensure proper management of administrator roles to prevent unauthorized temporal role assignments or revocations.
</Callout>
...
<LastUpdated date="2025-12-21T21:53:29.128Z" />
