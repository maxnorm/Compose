...
<DocSubtitle>
Manages paused roles and access control checks
</DocSubtitle>

<Callout type="info" title="Key Features">
- Internal functions for pausing and checking role status.
- Reverts with `AccessControlRolePaused` if a role is paused.
- Reverts with `AccessControlUnauthorizedAccount` if the account lacks the role.
- Integrates with diamond storage for shared state management.
</Callout>
...
This module provides functions to pause and unpause specific roles, enhancing access control within a diamond. The `requireRoleNotPaused` function checks both role membership and pause status, reverting with specific errors if conditions are not met. This ensures that even authorized accounts cannot execute actions associated with a paused role.
...
- Call `requireRoleNotPaused` to enforce both role membership and pause status.
- Use `pauseRole` and `unpauseRole` to manage operational states of specific roles.
- Ensure the `AccessControlPausableMod` is correctly initialized before use.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module utilizes diamond storage at the `ACCESS_CONTROL_STORAGE_POSITION` (defined as `keccak256("compose.accesscontrol")`) to manage `AccessControlPausableStorage`. All functions interact with this shared storage. Changes to role pause states are immediately visible to all facets that access the same storage position.
</Callout>
...
<LastUpdated date="2025-12-30T17:18:53.829Z" />
