...
- Provides `internal` functions for seamless integration with custom facets.
- Utilizes diamond storage for shared ownership state.
- Includes `renounceOwnership` to set the owner to address(0).
</Callout>
...
This module provides a two-step ownership transfer mechanism, ensuring secure transitions of contract control. Facets can import and utilize these functions to manage ownership, preventing accidental or unauthorized changes. Ownership changes are immediately reflected across all facets interacting with the shared diamond storage.
...
- Use `requireOwner()` within facets to restrict sensitive operations to the current owner.
- Handle `OwnerUnauthorizedAccount` errors when unauthorized callers attempt ownership actions.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module manages ownership state within the diamond's storage. It uses inline assembly to read and write to the `OWNER_STORAGE_POSITION` and `PENDING_OWNER_STORAGE_POSITION`, ensuring atomic updates visible to all facets. The `OwnerStorage` and `PendingOwnerStorage` structs define the layout for this state.
</Callout>
...
<LastUpdated date="2025-12-30T17:40:13.048Z" />
