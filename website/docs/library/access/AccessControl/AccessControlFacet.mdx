...
- Manages roles and their assignments to accounts.
- Provides functions to check, grant, and revoke roles.
- Supports batch operations for granting and revoking roles.
- Compatible with EIP-2535 diamond standard.
</Callout>

## Overview

This facet implements role-based access control for a diamond. It exposes functions to grant, revoke, and check roles, enabling granular permission management. Developers integrate this facet to enforce access policies on diamond functions.
...
// Example: Using AccessControlFacet within a diamond
contract MyDiamond {
    ...
    function grantAdminRole(address _account) public {
        // Assuming 'diamond' is the address of the diamond proxy
        // and the caller has the 'ROLE_ADMIN_ROLE' to execute this.
        diamond.grantRole("ROLE_ADMIN_ROLE", _account);
    }

    function checkUserRole(address _account, bytes32 _role) public view returns (bool) {
        return diamond.hasRole(_role, _account);
    }

    function enforceManagerRole(address _account) public view {
        diamond.requireRole("MANAGER_ROLE", _account);
    }
}`}
</ExpandableCode>
-->
...
- Enforce role checks on sensitive functions using `requireRole` or `hasRole`.
- Define and manage role hierarchies using `setRoleAdmin`.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
All role-modifying functions (`setRoleAdmin`, `grantRole`, `revokeRole`, `grantRoleBatch`, `revokeRoleBatch`) require the caller to have the admin role of the target role, enforced by `AccessControlUnauthorizedAccount` errors. The `renounceRole` function requires the caller to be the account from which the role is being renounced, enforced by `AccessControlUnauthorizedSender` errors. Input validation is handled by the underlying role management logic.
</Callout>
...
<LastUpdated date="2025-12-30T17:40:46.949Z" />
