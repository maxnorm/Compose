...
gitSource: "https://github.com/maxnorm/Compose/blob/7e2dd824639f424be644c4ebbb3ca2508167329f/src/access/AccessControl/AccessControlFacet.sol"
---
...
- Built-in checks to prevent unauthorized role modifications.
</Callout>

## Overview

The AccessControlFacet provides a robust role-based access control (RBAC) system for Compose diamonds. It allows for granular permission management, enabling administrators to grant and revoke roles from accounts. This facet is crucial for securing administrative functions and other sensitive operations within the diamond.
...
import {AccessControlFacet} from "@compose/diamond/contracts/facets/AccessControl/AccessControlFacet.sol";

contract DeployAccessControl {
    function deploy() external {
        // Assume diamondCutFacet is already deployed and accessible
        IDiamondCut diamondCutFacet = IDiamondCut(address(this)); // Replace with actual diamond address

        // Deploy the AccessControlFacet
        AccessControlFacet accessControlFacet = new AccessControlFacet();

        // Prepare facet cut to add AccessControlFacet
        IDiamondCut.FacetCut[] memory cut = new IDiamondCut.FacetCut[](1);
        cut[0] = IDiamondCut.FacetCut({
            facetAddress: address(accessControlFacet),
            action: IDiamondCut.FacetCutAction.ADD,
            selectors: AccessControlFacet.getSelectors()
        });

        // Execute the cut to add the facet to the diamond
        // diamondCutFacet.diamondCut(cut, address(0), ""); // Replace address(0) and "" with actual values if needed
    }

    // Example of calling a function after deployment
    function grantAdminRole(address _account, bytes32 _role) external {
        AccessControlFacet accessControlFacet = AccessControlFacet(address(this)); // Replace with actual diamond address
        accessControlFacet.grantRole(_role, _account);
    }
}`}
</ExpandableCode>
...
- Use `grantRoleBatch` and `revokeRoleBatch` for efficient management of multiple accounts.
- Regularly audit role assignments to ensure security and compliance.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Ensure that only trusted addresses are granted administrative roles. The `setRoleAdmin`, `grantRole`, and `revokeRole` functions are protected by access control checks, reverting if the caller is not the admin of the role. `renounceRole` is protected against unauthorized senders. No reentrancy concerns are apparent as state changes are atomic and do not involve external calls before state updates.
</Callout>
...
<LastUpdated date="2025-12-21T21:53:49.181Z" />
