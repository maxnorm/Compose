...
- No external dependencies or `using` directives, ensuring explicitness.
...
This module provides internal functions for role-based access control within a diamond. Facets import this module to manage and check account roles using shared diamond storage. Changes made through this module are immediately visible to all facets utilizing the same storage pattern.
...
- Ensure access control is enforced before calling internal functions like `grantRole` or `revokeRole`.
- Verify storage layout compatibility when upgrading facets to prevent storage collisions.
- Handle the `AccessControlUnauthorizedAccount` error when interacting with functions that perform role checks.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module uses diamond storage at the `STORAGE_POSITION` slot, identified by `keccak256("compose.accesscontrol")`. All functions within this module directly access and modify the `AccessControlStorage` struct. Changes made to roles are immediately visible to any facet that reads from this storage position, enabling composability and consistent access control across the diamond.
</Callout>
...
<LastUpdated date="2025-12-30T17:19:18.672Z" />
