...
<DocSubtitle>
Control role pausing and enforce role access
</DocSubtitle>

<Callout type="info" title="Key Features">
- `internal` functions for direct use within facets.
- Integrates with diamond storage pattern for shared state.
- Provides explicit role pausing and checking mechanisms.
- Reverts with custom errors for clear failure reasons.
</Callout>
...
This module enables granular control over role execution by allowing roles to be paused and unpaused. Facets can use this module to enforce that specific roles are active before executing sensitive operations, preventing unintended actions when a role is paused. It integrates seamlessly with diamond storage, ensuring consistent state across all facets.
...
import { AccessControlPausableMod, AccessControlPausableStorage } from "@compose/access/AccessControl/Pausable/AccessControlPausableMod";
import { AccessControlStorage } from "@compose/access/AccessControl/AccessControlStorage";

contract MyPausableFacet {
    AccessControlPausableMod internal accessControlPausableMod;

    function initialize(AccessControlPausableStorage storage accessControlPausableStorage, AccessControlStorage storage accessControlStorage) external {
        accessControlPausableMod = AccessControlPausableMod(accessControlPausableStorage, accessControlStorage);
    }

    function executeSensitiveAction(bytes32 _role, address _account) external {
        accessControlPausableMod.requireRoleNotPaused(_role, _account);
        // Sensitive action logic here
    }
...
- Use `requireRoleNotPaused` to ensure roles are active before critical operations.
- Call `pauseRole` and `unpauseRole` with appropriate access control.
- Handle `AccessControlRolePaused` and `AccessControlUnauthorizedAccount` errors explicitly.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module utilizes the diamond storage pattern, storing its state in a dedicated slot identified by `ACCESS_CONTROL_STORAGE_POSITION` (keccak256("compose.accesscontrol")). All functions interact directly with the `AccessControlPausableStorage` struct. Changes to role pausing status are immediately visible to any facet that accesses the same storage slot via the diamond proxy.
</Callout>
    ...
    items={[
      {
        ...
        description: "Facet using AccessControlPausableMod",
        icon: "💎"
      },
      {
...
<LastUpdated date="2026-03-12T19:40:39.933Z" />
