...
description: "Query diamond facets and their associated selectors."
...
<DocSubtitle>
Query diamond facets and their associated selectors.
</DocSubtitle>
...
- Enables programmatic discovery of diamond's functional surface area.
</Callout>

## Overview

The DiamondLoupeFacet provides essential introspection capabilities for a Compose diamond. It allows developers to query which facets are registered, the addresses of these facets, and the specific function selectors they implement. This facet is crucial for understanding the diamond's structure and for dynamic interaction.
...
import { IDiamondLoupe } from "@compose/contracts/diamond/facets/DiamondLoupeFacet.sol";
...
- Query facet information before attempting to call functions to ensure availability and understand implementation details.
- Use `facetAddresses()` and `facetFunctionSelectors()` to dynamically discover and verify function implementations within the diamond.
- Integrate facet discovery into deployment scripts or off-chain tooling for robust diamond management.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
This facet is read-only and does not modify state, thus posing minimal direct security risks. However, relying on its output for critical logic requires ensuring the diamond's state is consistent and that the queried selectors are indeed mapped to trusted facet addresses.
</Callout>
...
<LastUpdated date="2025-12-30T14:36:54.789Z" />
