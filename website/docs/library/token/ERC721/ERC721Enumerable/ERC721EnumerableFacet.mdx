...
- Full ERC-721 compliance including enumerable extensions.
- Efficient querying of token metadata, ownership, and balances.
- Support for `safeTransferFrom` to ensure receiver contract compatibility.
- Internal `transferFrom` function for composable internal logic.
</Callout>

## Overview

This facet implements the ERC-721 standard with enumerable extensions, allowing for efficient querying of token supply, balances, ownership, and token URIs. It provides core NFT functionalities alongside indexing capabilities essential for on-chain asset management within a Compose diamond.
...
import {IERC721Enumerable } from "@compose-protocol/contracts/facets/ERC721/IERC721Enumerable.sol";

contract ERC721EnumerableConsumer {
    IERC721Enumerable public nftFacet;

    constructor(address _diamondAddress) {
        // Assuming the diamond proxy is deployed and ERC721EnumerableFacet is added.
        nftFacet = IERC721Enumerable(_diamondAddress);
    }

    function getTokenName() external view returns (string memory) {
        return nftFacet.name();
    }

    function getTokenSymbol() external view returns (string memory) {
        return nftFacet.symbol();
    }

    function getTotalSupply() external view returns (uint256) {
        return nftFacet.totalSupply();
    }

    function getOwnerOfToken(uint256 _tokenId) external view returns (address) {
        return nftFacet.ownerOf(_tokenId);
    }
}
`}
</ExpandableCode>
...
- Initialize the facet with the correct storage layout and ensure it's correctly registered with the diamond proxy.
- When adding this facet, consider the storage slot usage to avoid collisions with other facets.
- Utilize `tokenOfOwnerByIndex` with caution, as it requires iteration and can be gas-intensive for large token holdings.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Access control for `approve`, `setApprovalForAll`, `transferFrom`, and `safeTransferFrom` functions is implicitly handled by the diamond's access control mechanism. Ensure proper role management is configured. Reentrancy is mitigated by the standard diamond proxy pattern and careful function implementation. Input validation is crucial for token IDs and addresses to prevent unintended state changes or errors.
</Callout>
...
<LastUpdated date="2025-12-30T14:36:01.379Z" />
