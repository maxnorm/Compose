...
description: "Manage enumerable ERC721 tokens within a diamond."
gitSource: "https://github.com/maxnorm/Compose/blob/7e2dd824639f424be644c4ebbb3ca2508167329f/src/token/ERC721/ERC721Enumerable/ERC721EnumerableMod.sol"
---
...
<DocSubtitle>
Manage enumerable ERC721 tokens within a diamond.
</DocSubtitle>

<Callout type="info" title="Key Features">
- Manages token enumeration for ERC721 standard.
- Integrates seamlessly with diamond storage for persistent state.
- Provides atomic operations for mint, burn, and transfer that update enumeration.
</Callout>
...
The ERC721EnumerableMod provides the core logic for managing enumerable ERC721 tokens. It ensures that tokens are correctly added to and removed from internal tracking lists during minting, burning, and transfers, making it easy to query token ownership and collections within a Compose diamond.
...
import {IERC721EnumerableMod, ERC721EnumerableMod} from "@compose/modules/ERC721EnumerableMod.sol";

contract MyERC721Facet {
    ERC721EnumerableMod internal enumerableMod;

    constructor(address _diamondAddress) {
        // Assuming ERC721EnumerableMod is deployed as a diamond facet
    ...
    function burnToken(uint256 _tokenId) external {
        address owner = enumerableMod.ownerOf(_tokenId); // Assumes ownerOf is implemented in another facet
        enumerableMod.burn(owner, _tokenId);
    }
...
- Ensure that the `ERC721EnumerableMod` facet is correctly initialized and accessible.
- Implement robust access control in your facets to gate calls to `mint` and `burn` appropriately.
- Always handle potential reverts from `mint`, `burn`, and `transferFrom` to maintain state integrity.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
The ERC721EnumerableMod interacts with diamond storage via a predefined slot for its internal `ERC721EnumerableStorage` struct. Facets calling this module should be aware of the storage layout and ensure no slot collisions occur. The module's functions modify internal state that is directly accessible to other facets through the diamond proxy, enabling composability for features like owner queries or token listing.
</Callout>
...
<LastUpdated date="2025-12-21T21:54:36.977Z" />
