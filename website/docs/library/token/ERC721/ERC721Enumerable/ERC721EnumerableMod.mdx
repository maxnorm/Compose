...
- Exposes `internal` functions for direct use within custom facets.
- Manages ERC-721 token state and enumeration using the diamond storage pattern.
- Does not rely on `using` directives for its core logic, promoting explicitness.
- Emits standard `Transfer` events upon token state changes.
</Callout>
...
This module provides internal functions for minting, burning, and transferring ERC-721 tokens, updating enumeration data. Facets importing this module can integrate ERC-721 functionality, leveraging shared diamond storage for token state. Changes made via this module are immediately visible to all facets accessing the same storage.
...
contract MyERC721Facet {
    using ERC721EnumerableMod for ERC721EnumerableMod;
    ...
    function burnToken(uint256 _tokenId, address _sender) external {
        ERC721EnumerableMod.burn(_tokenId, _sender);
    }

    function transferToken(address _from, address _to, uint256 _tokenId, address _sender) external {
        ERC721EnumerableMod.transferFrom(_from, _to, _tokenId, _sender);
    }
}`}
</ExpandableCode>
-->
...
- Ensure all calls to `mint`, `burn`, and `transferFrom` are guarded by appropriate access control mechanisms within your facet.
- Handle the specific errors emitted by this module, such as `ERC721NonexistentToken` or `ERC721InvalidReceiver`.
- Verify that the `STORAGE_POSITION` for `ERC721EnumerableStorage` is correctly reserved and not colliding with other facets.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module utilizes diamond storage at the predefined `STORAGE_POSITION` for `ERC721EnumerableStorage`. The `getStorage()` function provides inline assembly access to this storage struct. Any modifications made to the token state through `mint`, `burn`, or `transferFrom` functions are immediately reflected in the shared diamond storage, ensuring consistency across all facets interacting with this storage slot.
</Callout>
...
<LastUpdated date="2025-12-30T17:41:39.443Z" />
