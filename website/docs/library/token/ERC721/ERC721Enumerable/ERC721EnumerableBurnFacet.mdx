...
- Manages token enumeration state updates upon burning.
- Self-contained with no external dependencies.
</Callout>

## Overview

This facet enables burning ERC-721 tokens within a diamond. It provides the `burn` function, which removes a token and updates internal enumeration tracking. Developers integrate this facet to allow token destruction while maintaining the integrity of the token enumeration.
...
- Initialize the ERC721EnumerableStorage struct during diamond setup.
- Ensure the caller has the necessary approvals or ownership before burning a token.
- Verify storage compatibility when upgrading to new facet versions.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
The `burn` function requires appropriate checks for token ownership or operator approval. Input validation is crucial to prevent burning non-existent tokens, which would revert with `ERC721NonexistentToken`. Ensure caller authorization is handled correctly by the diamond's access control layer.
</Callout>
...
<LastUpdated date="2025-12-30T17:20:14.512Z" />
