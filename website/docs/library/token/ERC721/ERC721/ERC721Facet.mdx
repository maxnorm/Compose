...
sidebar_position: 2
title: "ERC721Facet"
description: "ERC-721 token management and transfers within a diamond"
...
<DocSubtitle>
ERC-721 token management and transfers within a diamond
</DocSubtitle>

<Callout type="info" title="Key Features">
- Exposes standard ERC-721 functions for external interaction via the diamond proxy.
- Manages token metadata (name, symbol, tokenURI) using diamond storage.
- Supports token transfers and approvals, including safe transfers.
- Self-contained, adhering to Compose facet design principles.
</Callout>

## Overview

This facet implements ERC-721 token functionality, including metadata retrieval and ownership management, as external functions within a diamond. It routes calls through the diamond proxy and accesses shared storage. Developers add this facet to expose ERC-721 capabilities while maintaining upgradeability and composability.
...
- Initialize the ERC721Storage struct during diamond deployment.
- Enforce ownership and approval checks on state-changing functions like `transferFrom` and `approve`.
- Ensure the receiver contract supports ERC-721 tokens when using `safeTransferFrom`.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Input validation is performed for token IDs, owners, and addresses. Functions like `ownerOf`, `getApproved`, and `balanceOf` revert with `ERC721NonexistentToken` or `ERC721IncorrectOwner` when conditions are not met. State-changing functions such as `transferFrom`, `safeTransferFrom`, and `approve` require appropriate ownership or approvals, reverting with errors like `ERC721InsufficientApproval` or `ERC721InvalidSender` if checks fail. Follow standard Solidity security practices for access control and reentrancy.
</Callout>
...
<LastUpdated date="2025-12-30T17:20:30.899Z" />
