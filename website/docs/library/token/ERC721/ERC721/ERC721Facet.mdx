...
gitSource: "https://github.com/maxnorm/Compose/blob/7e2dd824639f424be644c4ebbb3ca2508167329f/src/token/ERC721/ERC721/ERC721Facet.sol"
---
...
- Full ERC-721 compliance for NFT management.
- Supports token metadata retrieval via `tokenURI`.
- Includes both internal and external transfer functions for flexibility.
- Handles approvals and operator authorizations.
</Callout>

## Overview

The ERC721Facet provides a standard interface for managing Non-Fungible Tokens (NFTs) within a Compose diamond. It handles token ownership, transfers, approvals, and retrieval of token metadata, adhering to the ERC-721 standard.
...
import {IERC721Facet} from "@compose/contracts/facets/ERC721Facet.sol";

contract ERC721Consumer {
    IERC721Facet public erc721Facet;

    constructor(address _diamondProxy) {
        erc721Facet = IERC721Facet(_diamondProxy);
    }
    ...
    function transferMyToken(address to, uint256 tokenId) external {
        // Assumes caller is owner and has approved the diamond proxy
...
- Initialize the `ERC721Facet` with the correct storage slot pointer during diamond deployment.
- Use `approve` and `setApprovalForAll` to grant necessary permissions before calling transfer functions.
- Ensure the diamond proxy has the necessary role or permissions to execute these functions if access control is implemented at the diamond level.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
This facet implements standard ERC-721 logic. Security relies on correct diamond proxy implementation and any access control layers. Ensure proper checks are in place for `transferFrom` and `safeTransferFrom` to prevent unauthorized token movement. Reentrancy is mitigated by internal checks and the diamond proxy pattern. Input validation is handled by custom errors like `ERC721InvalidOwner` and `ERC721NonexistentToken`.
</Callout>
...
<LastUpdated date="2025-12-21T21:54:57.380Z" />
