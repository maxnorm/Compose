...
- Manages token balances, allowances, and operator status using diamond storage.
...
This facet implements ERC-6909 token functionality within a diamond. It provides external functions for token transfers, allowances, and operator management, routing calls through the diamond proxy and accessing shared storage. Developers integrate this facet to expose composable token capabilities while preserving diamond upgradeability.
...
- Initialize the ERC6909Storage struct at the correct storage slot during diamond deployment.
- Ensure that the `setOperator` function is used to grant necessary permissions for `transferFrom` operations.
- Validate input parameters in functions like `transfer` and `transferFrom` to prevent unexpected behavior and gas wastage.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
The `transfer` and `transferFrom` functions implement checks-effects-interactions pattern to mitigate reentrancy risks. Input validation is performed for receiver and sender addresses. Granting operator status via `setOperator` requires careful consideration of the approved spender's capabilities. Insufficient balance or allowance will revert with `ERC6909InsufficientBalance` or `ERC6909InsufficientAllowance` respectively.
</Callout>
...
<LastUpdated date="2025-12-30T17:41:32.587Z" />
