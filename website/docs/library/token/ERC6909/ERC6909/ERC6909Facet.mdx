...
<DocSubtitle>
ERC-6909 token functionality within a diamond
</DocSubtitle>

<Callout type="info" title="Key Features">
- Implements ERC-6909 token transfer and approval functions.
- Utilizes diamond storage for shared state management.
- Exposes external functions for direct diamond interaction.
- No external dependencies beyond diamond proxy and storage.
</Callout>

## Overview

This facet implements ERC-6909 token functionality as external functions callable through a diamond proxy. It accesses shared diamond storage to manage token balances, allowances, and operator approvals. Developers integrate this facet to expose token operations while leveraging the diamond's upgradeability and composability.
...
contract ExampleDiamondUser {
    ...
    function setOperatorStatus(address _spender, bool _approved) external {
...
- Initialize ERC6909 storage correctly before calling any state-changing functions.
- Ensure the caller has sufficient balance or allowance before attempting transfers.
- Verify that operator approvals are managed securely and revoked when necessary.
</Callout>
...
<LastUpdated date="2026-03-13T01:52:54.834Z" />
