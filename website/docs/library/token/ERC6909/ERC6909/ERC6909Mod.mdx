...
- Exposes `internal` functions for mint, burn, transfer, and approve operations.
- Utilizes the diamond storage pattern at a dedicated `STORAGE_POSITION`.
- No external dependencies, promoting composability.
- Provides error types for common ERC-6909 failures.
</Callout>
...
This module provides internal functions for managing ERC-6909 compliant multi-tokens. Facets can import this module to interact with token state stored within the diamond using a shared storage pattern. It enables composable token logic by exposing core operations like minting, burning, transferring, and approvals.
...
contract MyTokenFacet {
    ...
    function transferTokens(address _from, address _to, uint256 _id, uint256 _amount) external {
        ...
        storagePtr.transfer(_from, _from, _to, _id, _amount);
    }
...
- Ensure the caller has sufficient balance and allowance before initiating transfers.
- Verify that receiver addresses are valid and not zero addresses.
- Handle specific errors such as `ERC6909InsufficientBalance` and `ERC6909InsufficientAllowance`.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module accesses shared state via diamond storage at the slot identified by `keccak256("erc6909")`. All functions are internal and operate directly on the `ERC6909Storage` struct, ensuring that changes are immediately visible to all facets interacting with this storage position.
</Callout>
...
<LastUpdated date="2026-03-12T19:44:27.955Z" />
