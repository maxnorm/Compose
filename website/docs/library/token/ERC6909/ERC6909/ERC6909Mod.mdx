...
- Supports minting, burning, transferring, and approving tokens.
- Manages operator relationships for delegated transfers.
- Includes robust error handling for common ERC-6909 scenarios.
</Callout>
...
The ERC6909Mod module provides the core internal logic and storage for implementing the ERC-6909 standard. It enables managing balances, approvals, and operator permissions for multiple token types within a single diamond, promoting efficient storage and composability.
...
import {IERC6909Mod, IERC6909ModStorage} from "./interfaces/IERC6909Mod.sol";

contract ERC6909Facet {
    // Assume STORAGE_POSITION is defined and accessible
    // Assume diamond storage is accessible

    function transferToken(address _from, address _to, uint256 _id, uint256 _amount) external {
        IERC6909ModStorage storage storagePtr = IERC6909ModStorage(STORAGE_POSITION);
        IERC6909Mod(address(storagePtr)).transfer(_from, _to, _id, _amount);
    }

    function approveToken(address _spender, uint256 _id, uint256 _amount) external {
        IERC6909ModStorage storage storagePtr = IERC6909ModStorage(STORAGE_POSITION);
        IERC6909Mod(address(storagePtr)).approve(_spender, _id, _amount);
    }

    function burnToken(address _from, uint256 _id, uint256 _amount) external {
        IERC6909ModStorage storage storagePtr = IERC6909ModStorage(STORAGE_POSITION);
        IERC6909Mod(address(storagePtr)).burn(_from, _id, _amount);
    }
}`}
</ExpandableCode>
...
- Ensure correct initialization of the ERC6909Mod storage structure.
- Handle potential `ERC6909InsufficientBalance` and `ERC6909InsufficientAllowance` errors appropriately.
- Be mindful of operator permissions when implementing `transfer` logic.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module requires a dedicated storage slot, defined by `STORAGE_POSITION`, to hold its internal `ERC6909ModStorage` struct. Facets interacting with this module must resolve `STORAGE_POSITION` to access the module's state and functions. All state modifications are performed directly within the diamond's storage context, making them immediately visible to other facets.
</Callout>
...
<LastUpdated date="2025-12-30T14:35:49.931Z" />
