...
<DocSubtitle>
Manage ERC-1155 token balances, transfers, and URIs.
</DocSubtitle>

<Callout type="info" title="Key Features">
- Supports both single and batch operations for minting, burning, and transfers, optimizing for efficiency.
- Enforces ERC-1155 safe transfer requirements, including receiver validation for contract recipients.
- Provides functionality to manage token metadata via base URI and token-specific URIs.
</Callout>
...
This module provides a comprehensive implementation for ERC-1155 multi-token standards. It enables minting, burning, and safe transfers of both single and batch token types, while also managing token URIs. This module is essential for any diamond that handles fungible or non-fungible tokens under the ERC-1155 standard.
...
contract MyFacet {
    ...
    function mintTokens(address to, uint256 id, uint256 amount) external {
        // Ensure caller has permission to mint (e.g., via AccessControlFacet)
        ERC1155.mint(to, id, amount);
    }

    function transferTokens(address from, address to, uint256 id, uint256 amount) external {
        // Ensure caller has permission to transfer (e.g., via AccessControlFacet or ownership)
        ERC1155.safeTransferFrom(from, to, id, amount);
    }

    function burnTokens(address from, uint256 id, uint256 amount) external {
        // Ensure caller has permission to burn
...
- Implement robust access control for minting and burning functions, ensuring only authorized addresses can perform these actions.
- Always validate recipient contract addresses when performing safe transfers to ensure they implement the ERC1155TokenReceiver interface to prevent token loss.
- Be mindful of gas costs when performing batch operations; consider the trade-offs between single operations and batch operations based on expected usage patterns.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
The ERC1155Mod uses a predefined storage slot within the diamond's storage to manage its state, including token balances, approvals, and URI mappings. Facets interacting with this module should be aware that any function that modifies balances or URIs will be reflected globally across all facets that can call the ERC1155 functions. The `getStorage` function provides direct access to the ERC1155 storage struct, allowing for inspection of the module's internal state.
</Callout>
...
<LastUpdated date="2025-12-30T14:35:36.699Z" />
