...
- Internal functions for ERC-1155 token management.
- Supports safe transfers for both single and batch operations.
...
This module provides internal functions for managing ERC-1155 token balances, transfers, and metadata within a diamond. Facets can import this module to implement ERC-1155 compliant functionality, leveraging shared diamond storage for token state. It ensures safe transfers by validating receiver contracts and handles batch operations for efficiency.
...
contract MyERC1155Facet {
    ...
    function initialize(address erc1155ModAddress) external {
        erc1155Mod = ERC1155Mod(erc1155ModAddress);
    }

    function mintTokens(address _to, uint256 _id, uint256 _value) external {
        // Assumes caller has permission to mint.
        erc1155Mod.mint(_to, _id, _value);
    }

    function transferTokens(address _from, address _to, uint256 _id, uint256 _value, address _operator) external {
        // Assumes caller has permission to transfer.
        erc1155Mod.safeTransferFrom(_from, _to, _id, _value, _operator);
    }

    function burnTokens(address _from, uint256 _id, uint256 _value) external {
        // Assumes caller has permission to burn.
...
- Ensure necessary approvals are granted before calling transfer functions.
- Perform receiver validation when implementing custom mint or transfer logic.
- Handle specific errors like `ERC1155InsufficientBalance` and `ERC1155InvalidReceiver`.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module interacts with diamond storage at the `STORAGE_POSITION` defined by `keccak256("compose.erc1155")`. It utilizes the `ERC1155Storage` struct to manage token URIs and base URIs. All state changes made through this module are immediately visible to any facet accessing the same storage position.
</Callout>
...
<LastUpdated date="2025-12-30T17:41:21.383Z" />
