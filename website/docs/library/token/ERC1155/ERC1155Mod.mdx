...
- Implements core ERC-1155 transfer and mint/burn logic.
- Manages token metadata including base and token-specific URIs.
- Utilizes diamond storage for shared state management.
- Supports safe transfers with receiver validation.
</Callout>
...
This module provides core ERC-1155 functionality, including minting, burning, and transfers, all managed through the diamond storage pattern. Facets can import this module to implement ERC-1155 compliant behavior, ensuring consistent token management across the diamond. It supports token metadata through base URIs and token-specific URIs.
...
- Ensure the `ERC1155Mod` is correctly initialized with the diamond's storage pointer.
- Verify receiver addresses are valid contracts before performing safe transfers to prevent `ERC1155InvalidReceiver` errors.
- Handle potential `ERC1155InsufficientBalance` errors when performing burns or transfers.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module interacts with diamond storage at the position defined by `keccak256(\"compose.erc1155\")`. All state changes, such as token balances and URIs, are written to this storage slot. The `ERC1155Storage` struct, which contains fields like `uri` and `baseURI`, is used to manage this state. Access to this storage is managed internally, ensuring all facets using this module operate on the same consistent state.
</Callout>
...
<LastUpdated date="2025-12-30T17:19:52.847Z" />
