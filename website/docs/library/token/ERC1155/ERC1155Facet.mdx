...
- Supports ERC-1155 standard for multiple token types.
- Includes batch transfer and balance checking capabilities.
- Provides URI resolution for token metadata.
</Callout>

## Overview

The ERC1155Facet provides full support for the ERC-1155 multi-token standard within a Compose diamond. It enables the management and transfer of multiple fungible and non-fungible token types from a single contract, facilitating complex asset management scenarios.
...
import {IERC1155Facet} from "@compose/core/src/facets/ERC1155/IERC1155Facet.sol";
...
contract MyDiamond is IERC1155Facet {
    ...
    function uri(uint256 _id) external view override returns (string memory) {
        // Logic to determine URI based on token ID
        // Example: return "ipfs://my_metadata/{id}";
    ...
    function balanceOf(address _owner, uint256 _id) external view override returns (uint256) {
        return ERC1155Facet(address(this)).balanceOf(_owner, _id);
    }

    function safeTransferFrom(address _from, address _to, uint256 _id, uint256 _value, bytes memory _data) external override {
        ERC1155Facet(address(this)).safeTransferFrom(_from, _to, _id, _value, _data);
    }

    // ... other ERC1155 functions
}`}
</ExpandableCode>
...
- Initialize the base URI and token-specific URIs during deployment or via admin functions.
- Ensure proper access control is implemented for functions that modify approvals or transfer tokens.
- Leverage `balanceOfBatch` and `safeBatchTransferFrom` for gas efficiency when performing operations on multiple token types or accounts.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Implement robust access control for `setApprovalForAll` to prevent unauthorized approvals. Carefully validate `_from`, `_to`, `_id`, and `_value` in transfer functions to prevent unexpected state changes. Ensure reentrancy guards are in place if external calls are made within transfer functions.
</Callout>
...
<LastUpdated date="2025-12-30T14:35:38.461Z" />
