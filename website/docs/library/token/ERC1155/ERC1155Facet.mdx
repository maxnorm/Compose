...
sidebar_position: 2
title: "ERC1155Facet"
description: "Manages ERC-1155 tokens within a diamond"
...
<DocSubtitle>
Manages ERC-1155 tokens within a diamond
</DocSubtitle>

<Callout type="info" title="Key Features">
- Exposes external functions for ERC-1155 token operations via diamond routing.
...
- Emits standard ERC-1155 events for tracking token activity.
</Callout>

## Overview

This facet implements ERC-1155 token functionality as external functions within a diamond. It provides standard ERC-1155 operations, routing calls through the diamond proxy and interacting with shared diamond storage. Developers add this facet to expose fungible and non-fungible token capabilities while preserving upgradeability.
...
// Example: Calling ERC1155 functions through the diamond proxy
contract DiamondUser {
    ...
    function transferSingleToken(address _to, uint256 _id, uint256 _value, bytes _data) external {
        // Calls safeTransferFrom on the ERC1155Facet via the diamond
        diamond.safeTransferFrom(address(this), _to, _id, _value, _data);
    }

    function getBalance(address _account, uint256 _id) external view returns (uint256) {
        // Calls balanceOf on the ERC1155Facet via the diamond
        return diamond.balanceOf(_account, _id);
    }

    function approveOperator(address _operator, bool _approved) external {
        // Calls setApprovalForAll on the ERC1155Facet via the diamond
...
<LastUpdated date="2025-12-30T20:08:32.596Z" />
