...
- Provides `crosschainBurn` and `crosschainMint` functions for cross-chain operations.
- Enforces `trusted-bridge` role using internal access control checks.
- Utilizes diamond storage for ERC20 state management.
...
This module enables cross-chain token transfers by providing functions for minting and burning tokens on behalf of trusted bridge addresses. It integrates with diamond storage to manage ERC20 state and access control roles. Facets importing this module can delegate cross-chain operations to authorized bridge contracts.
...
contract MyFacet {
    ERC20BridgeableMod private erc20Bridgeable;

    constructor(address _diamondProxy) {
        erc20Bridgeable = ERC20BridgeableMod(_diamondProxy);
    }

    /**
     * @notice Burns tokens for a cross-chain operation.
     * @param _from The address to burn tokens from.
     * @param _value The amount of tokens to burn.
     */
    function burnForCrosschain(address _from, uint256 _value) external {
        // The caller must have the 'trusted-bridge' role for crosschainBurn to succeed
        // The ERC20BridgeableMod contract enforces this role check internally.
        erc20Bridgeable.crosschainBurn(_from, _value);
    }

    /**
     * @notice Mints tokens for a cross-chain operation.
     * @param _account The address to mint tokens to.
     * @param _value The amount of tokens to mint.
     */
    function mintForCrosschain(address _account, uint256 _value) external {
        // The caller must have the 'trusted-bridge' role for crosschainMint to succeed
        // The ERC20BridgeableMod contract enforces this role check internally.
        erc20Bridgeable.crosschainMint(_account, _value);
    }
}`}
</ExpandableCode>
-->
...
- Ensure the caller has the `trusted-bridge` role before calling `crosschainBurn` or `crosschainMint`.
- Verify that the `ERC20_STORAGE_POSITION` is correctly set in the diamond storage.
- Handle the `ERC20InvalidBridgeAccount` error if the bridge account is invalid.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module interacts with diamond storage at the `ERC20_STORAGE_POSITION`, which is determined by `keccak256("compose.erc20")`. It relies on the `AccessControlStorage` struct for role management and `ERC20Storage` for token state. All state modifications are immediately visible to other facets that access the same storage slots.
</Callout>
...
<LastUpdated date="2025-12-30T17:41:13.627Z" />
