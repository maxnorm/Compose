...
- Restricts mint/burn operations to addresses with the `trusted-bridge` role.
- Integrates with the diamond storage pattern for shared state access.
- Provides internal helper functions for storage retrieval.
</Callout>

## Overview

This facet enables cross-chain minting and burning of ERC-20 tokens within a diamond. It enforces access control for trusted bridge operators and provides functions to manage token balances across different chains. Developers integrate this facet to facilitate secure and upgradeable cross-chain token operations.
...
contract Deployer {
    ...
    function checkBridge(address _caller) external view {
        IDiamond diamond = IDiamond(DIAMOND_ADDRESS);
        diamond.checkTokenBridge(_caller);
    }
}`}
</ExpandableCode>
-->
...
- Call `crosschainMint` and `crosschainBurn` only from authorized bridge contracts or services.
- Verify the `checkTokenBridge` function logic aligns with your cross-chain security requirements.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
Cross-chain operations require careful security considerations. The `crosschainMint` and `crosschainBurn` functions are protected by the `trusted-bridge` role via the `checkTokenBridge` internal check. Reentrancy is mitigated by following the checks-effects-interactions pattern. Input validation is performed on recipient and sender addresses. Ensure the underlying ERC20 token implementation does not introduce additional vulnerabilities.
</Callout>
...
<LastUpdated date="2025-12-30T17:41:19.177Z" />
