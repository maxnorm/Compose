...
- Enforces access control for `trusted-bridge` role on sensitive functions.
- Integrates with diamond storage for token state management.
- Provides a function to verify trusted bridge addresses.
</Callout>

## Overview

This facet enables cross-chain minting and burning of tokens within a diamond. It exposes external functions that are callable only by addresses holding the `trusted-bridge` role, ensuring secure cross-chain operations. This facet integrates with the diamond's access control and token storage.
...
- Initialize the `trusted-bridge` role for authorized cross-chain operators.
- Call `crosschainMint` and `crosschainBurn` only from secure, whitelisted bridge contracts.
- Use `checkTokenBridge` to verify the legitimacy of a caller before interacting with cross-chain operations.
</Callout>

## Security Considerations

<Callout type="warning" title="Security">
The `crosschainMint` and `crosschainBurn` functions are protected by the `trusted-bridge` role, preventing unauthorized cross-chain operations. The `checkTokenBridge` function validates callers against this role. Follow standard Solidity security practices for input validation and reentrancy.
</Callout>
...
<LastUpdated date="2025-12-30T17:19:50.053Z" />
