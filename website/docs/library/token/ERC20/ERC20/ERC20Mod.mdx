...
- Provides `internal` functions for ERC-20 operations.
- Leverages diamond storage (EIP-8042) for state management.
...
This module exposes internal functions for core ERC-20 token operations such as minting, burning, transferring, and approving allowances. Facets can import and use these functions to manage token logic within the diamond storage pattern, ensuring consistency and reducing storage collisions. All operations directly interact with shared diamond storage.
...
- Call `mint`, `burn`, `transfer`, `transferFrom`, and `approve` only after validating necessary permissions or allowances.
- Handle custom errors like `ERC20InsufficientBalance` and `ERC20InsufficientAllowance` to ensure robust execution.
- Ensure the `ERC20Storage` layout is compatible when upgrading facets to maintain state integrity.
</Callout>

## Integration Notes

<Callout type="success" title="Shared Storage">
This module utilizes the diamond storage pattern, storing ERC-20 state at the `STORAGE_POSITION` defined by `keccak256("compose.erc20")`. All internal functions directly read from and write to the `ERC20Storage` struct at this designated slot. Changes to token balances, total supply, or allowances made through this module are immediately visible to all other facets interacting with the same diamond storage.
</Callout>
...
<LastUpdated date="2025-12-30T17:19:36.910Z" />
